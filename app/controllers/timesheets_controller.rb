...
class TimesheetsController < ApplicationController # rubocop:disable Style/Documentation
  before_action :show
  def index; end
  ...
  def show # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/MethodLength,Metrics/PerceivedComplexity
    ...
    is_supervisor = current_user.employee.current_supervisees.collect(&:supervisee).include?(@timesheet.employee_id)
    ...
    @projects = Project.where(project_id: records.collect(&:project_id).uniq).collect do |x|
      [x.project_id, x.short_name]
    end.to_h
  ...
  def reopen_timesheet
    next_state = WorkflowState.where(state: 'Re-opened',
                                     workflow_process_id: WorkflowProcess.find_by_workflow('Timesheet').id).first
    @timesheet = Timesheet.find(params[:id]).update(submitted_on: nil, approved_by: nil, approved_on: nil,
                                                    state: next_state.id)
    redirect_to "/time_sheets/#{params[:id]}"
  end
